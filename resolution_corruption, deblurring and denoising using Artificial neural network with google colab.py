# -*- coding: utf-8 -*-
"""IMPR_Ex5_work_for_quick_mode.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1AdyOx7Tmqt87J-WCjQAr9jWgVA6YtxZZ
...
"""###Version 1.3 - Last Update 14.01.2021
**Changelog:** 
* Change the due date 

**Version 1.2 - 10.01.2021 Changelog:** 
...
#@markdown ##Due date: 26.01.2021 at 23:59
#@title{ display-mode: "form" }
#@markdown Please enter your cs username:
#cs_id = "amitchen93" #@param {type:"string"}
...
def load_dataset(filenames, batch_size, corruption_func, crop_size):
    ...
    """
    # calc (height, width) of crop_size:
    height, width = crop_size
    size_one = 1
    half_size = 0.5

    # respective randomly corrupted version according to corruption_func(im)
    source_batch = np.zeros((batch_size, height, width, size_one))

    # made of clean images
    target_batch = np.zeros((batch_size, height, width, size_one))

    #  You can use a simple dictionary  for this process, mapping filenames to numpy arrays of images
    images_dict = {}  # empty dict

    while True:
        for index in range(batch_size):
            # Each im in the batch should be picked at random from the given list
            len_of_file = len(filenames)
            # randrange returns a randomly selected element from specified range.
            random_selected_image = randrange(len_of_file)

            if random_selected_image not in images_dict:
                images_dict[random_selected_image] = read_image(filenames[random_selected_image], 1)
            # else the random_selected_image is in images (no need to read_image)
            image = images_dict[random_selected_image]

            # we should instead first sample a larger random crop, of size 3 × crop_size
            triple_height = height * 3
            triple_width = width * 3

            height_3 = randrange(image.shape[0] - triple_height)
            width_3 = randrange(image.shape[1] - triple_width)

            patch = image[height_3: height_3 + (triple_height),
                    width_3: width_3 + (triple_width)]

            # followed by corrupting the entire image with corruption_func(im)
            # randomly choosing the location of a patch the size of crop_size
            real_height = randrange(patch.shape[0] - height)
            real_width = randrange(patch.shape[1] - width)

            real_height_plus_height = real_height + height
            real_width_plus_width = real_width + width

            # subtract value 0.5 from each pixel in both source and target patches
            source = corruption_func(patch)[real_height: real_height_plus_height,
                     real_width: real_width_plus_width].reshape((height, width, size_one)) - half_size
            source_batch[index] = np.clip(source, -1 * half_size, half_size)

            target = patch[real_height: real_height_plus_height,
                     real_width: real_width_plus_width].reshape((height, width, size_one)) - half_size
            target_batch[index] = np.clip(target, -1 * half_size, half_size)
...
def resblock(input_tensor, num_channels):
    ...
    """
    # we first feed X to a 3×3 convolution
    three_const = 3
    temp_output = Conv2D(num_channels, (three_const, three_const), padding='same')(input_tensor)  # todo do pad?

    # followed by ReLU activation
    temp_output = Activation('relu')(temp_output)

    # then another 3×3 convolution - almost the same as before
    temp_output = Conv2D(num_channels, (three_const, three_const), padding='same')(temp_output)  # todo do pad?

    # denote it’s output with O ('boost' as Avital teach us)
    temp_output = Add()([input_tensor, temp_output])

    # The final output of the residual block is ReLU(O + X), connecting the input to the last output
    temp_output = Activation('relu')(temp_output)
    return temp_output
...
def build_nn_model(height, width, num_channels, num_res_blocks):
    ...
    """
    # you can only use the the following Keras’ classes/methods to construct the
    # model: Model, Conv2D, Activation, Input, Add. You are not allowed to use the Sequential API!

    # with input dimension the shape of (height, width, 1)
    one_const = 1
    step_one = Input(shape=(height, width, one_const))

    # all convolutional layers (including residual blocks) with number of output channels equal to num_channels
    three_const = 3
    step_two = Conv2D(num_channels, (three_const, three_const), padding='same')(step_one)  # todo do pad?
    step_two_act = Activation('relu')(step_two)

    # The number of residual blocks should be equal to num_res_blocks.
    for block_i in range(num_res_blocks):
        step_two_res_block = resblock(step_two_act, num_channels)

    step_two = Conv2D(one_const, (three_const, three_const), padding='same')(step_two_res_block)  # todo do pad?
    step_two_add = Add()([step_two, step_one])  # ('boost' as Avital teach us)

    return Model(inputs=step_one, outputs=step_two_add)
...
def train_model(model, images, corruption_func, batch_size, steps_per_epoch, num_epochs, num_valid_samples):
    ...
    """
    # It should divide the images into a training set and validation set, using an 80-20 split
    len_of_im = len(images)
    train_part, valid_part = train_test_split(images, test_size=0.2, random_state=42)  # 80% of images, rest 20% of images

    one_const = 1
    three_const = 3
    cut_model = model.input_shape[one_const: three_const]

    # generate from each set a dataset with the given batch size and corruption function (load_dataset)
    generator_for_train_set = load_dataset(train_part, batch_size, corruption_func, cut_model)
    generator_for_valid_set = load_dataset(valid_part, batch_size, corruption_func, cut_model)

    # you should call the compile() method of the model using the “mean squared error” loss and ADAM optimizer
    # Instead of the default values for ADAM, import the class Adam from keras.optimizers, and use Adam(beta_2=0.9)
    model.compile(loss='mean_squared_error', optimizer=Adam(beta_2=0.9))

    # Finally, you should call fit_generator to actually train the model
    model.fit_generator(generator_for_train_set, steps_per_epoch=steps_per_epoch,
                        epochs=num_epochs, validation_data=generator_for_valid_set,
...
def restore_image(corrupted_image, base_model):
    ...
    corrupted_image = np.expand_dims(corrupted_image, axis=2)
    corrupted_image = corrupted_image - const_half


    # create a new model that its the size of the input image and has the same weights as the given base model
    a = Input(shape=corrupted_image.shape)  # with width and height of the image to be restored.
    b = base_model(a)
    new_model = Model(inputs=a, outputs=b)

    # before using the predict() method to restore the image.
    predict_image = new_model.predict(corrupted_image[np.newaxis, ...])[0] + const_half
    reshape_predict_image = predict_image.reshape(predict_image.shape[0], predict_image.shape[1])

    # Finally, you will need to clip the results to the [0; 1] range.
    result = np.clip(reshape_predict_image, const_zero, const_one).astype(np.float64)
...
def add_gaussian_noise(image, min_sigma, max_sigma):
    ...
    sigma_val = np.random.uniform(min_sigma, max_sigma)

    # adding to every pixel of the input image a zero-mean gaussian random variable with sd equal to sigma.
    # numpy.random.normal
    shape_of_image = image.shape
    noise_variable = np.random.normal(loc=0, scale=sigma_val, size=shape_of_image)

    # the values should be rounded to the nearest fraction i 255 and clipped to [0; 1].
    sum_of_img_noise = image + noise_variable
    corrupted_image = np.clip(np.around(sum_of_img_noise * 255) / 255, const_zero, const_one).astype(np.float64)
...
def learn_denoising_model(denoise_num_res_blocks, quick_mode=False):
    ...
    """
    # train a network which expects patches of size 24x24, using 48 channels for all but the last layer.
    patch_len = 24
    num_of_channels = 48
    model = build_nn_model(patch_len, patch_len, num_of_channels, denoise_num_res_blocks)
    # todo denoise_num_res_blocks as parameter?

    # Use function images_for_denoising() we supplied for you to get the image paths list for training on this task
    images = images_for_denoising()

    # use 100 images in batch, 100 steps per epoch, 10 epochs overall, 1000 samples for testing on the validation set.
    batch_num = 100
    steps_in_epoch = 100
    epoch_num = 10
    samples_num = 1000

    # If quick_mode equals True, instead of the above arguments, use only 10 images in a batch, 3 steps per epoch,
    # just 2 epochs and only 30 samples for the validation set.
    if quick_mode == True:
        batch_num = 10
        steps_in_epoch = 3
        epoch_num = 2
    ...
    # you can wrap the function add_gaussian_noise in a lambda expression / local function
    def local_function(image):
        min_range = 0
        max_range = 0.2
        add_noise = add_gaussian_noise(image, min_range, max_range)
        return add_noise

    # now train:
    train_model(model, images, local_function,
                batch_size=batch_num,
                steps_per_epoch=steps_in_epoch,
                num_epochs=epoch_num,
                num_valid_samples=samples_num)
...
def add_motion_blur(image, kernel_size, angle):
    ...
    """
    # we provide you with the function motion_blur_kernel(kernel_size, angle) for generating this kernel,
    kernel = motion_blur_kernel(kernel_size, angle)

    # apply to the image with scipy.ndimage.filters.convolve.
...
def random_motion_blur(image, list_of_kernel_sizes):
    ...
    :return: blurred image
    """
    # samples an angle at uniform from the range [0; pi].
    ...
    blurred_image = add_motion_blur(image, kernel_size, random_angle)

    # it should be rounded to the nearest fraction and and clipped to [0; 1]
    blurred_image = np.clip(np.around(blurred_image * 255) / 255, 0, 1).astype("float64")
...
def learn_deblurring_model(deblur_num_res_blocks, quick_mode=False):
    ...
    :return: the trained model
    """
    # The above method should train a network which expect patches of size 16x16, and have
    # 32 channels in all layers except the last.
    patch_size = 16
    channels_size = 32

    # Use images_for_deblurring() we supplied for you to get the image paths list for training on this task
    image_path_list = images_for_deblurring()

    # The dataset should use a random motion blur of kernel size equal to 7
    trained_model = build_nn_model(patch_size, patch_size, channels_size, num_res_blocks=deblur_num_res_blocks)  # todo 7?

    # use 100 images in a batch, 100 steps epoch, 10 epochs overall, 1000 samples for testing on the validation set.
    batch_size = 100
    steps_per_epoch = 100
    num_epochs = 10
    num_valid_samples = 1000

    # instead of the above arguments, use 10 images in a batch, 3 steps per epoch,
    # just 2 epochs and only 30 samples for the validation set.
    if quick_mode == True:
        batch_size = 10
        steps_per_epoch = 3
    ...
    # you can wrap the function random_motion_blur in use a local function.
    def apply_random_motion_blur(image):
        return random_motion_blur(image, [7]) # todo 7?

    train_model(trained_model, image_path_list, apply_random_motion_blur,
...
def super_resolution_corruption(image):
    ...
    """
    # scipy.ndimage.zoom function may come in handy.
    random_int = np.random.randint(1, 4) 
    inv_int = (1 / random_int) # inv

    # now zoom in and then zoom out
    zoom_in_image = zoom(image, inv_int) # zoom out
    corrupted_image = zoom(zoom_in_image, random_int) # zoom in
...
def learn_super_resolution_model(super_resolution_num_res_blocks, quick_mode=False):
    ...
    # Use the function images_for_super_resolution() we supplied for you to get the image paths list for training
    image_path_list = images_for_super_resolution()

    # The dataset should use a random motion blur of kernel size equal to 7
    trained_model = build_nn_model(patch_size, patch_size, channels_size,
                                   num_res_blocks=super_resolution_num_res_blocks)

    # experiment with the parameters and use the ones that produce the best visual results for you
    batch_size = 100
    steps_per_epoch = 100
    num_epochs = 10

    num_valid_samples = 1000  # it's good - Use a 1000 samples for the testing validation set.
    ...
    # you can wrap the function random_motion_blur in use a local function.
    def apply_super_resolution_corruption(image):
        return super_resolution_corruption(image)  # todo this fuction should be here?

    train_model(trained_model, image_path_list, apply_super_resolution_corruption,
                batch_size=batch_size,
                steps_per_epoch=steps_per_epoch,
                num_epochs=num_epochs,
                num_valid_samples=num_valid_samples)
...
# lst_of_super = []
# for i in range(1, 6):
#     model = learn_denoising_model(i, quick_mode=False)
#     lst_of_denoise.append(model.history.history['val_loss'])
#
#     model = learn_deblurring_model(deblur_num_res_blocks, quick_mode=False)
#     lst_of_blur.append(model.history.history['val_loss'])
#
#     model = learn_super_resolution_model(i, quick_mode=False)
#     lst_of_super.append(model.history.history['val_loss'])
#
#     plt.xlabel("epochs")
#     plt.ylabel("loss of validation")
#     plt.title("de-noise loss for resblocks num")
#     for i in range(len(lst_of_denoise)):
#         plt.plot(range(len(lst_of_denoise[i])), lst_of_denoise[i], label='CNN with {} resblocks'.format(i+1))
#     plt.legend
...
#     plt.xlabel("epochs")
#     plt.ylabel("loss of validation")
#     plt.title("deblur loss for resblocks num")
#     for i in range(len(lst_of_blur)):
#         plt.plot(range(len(lst_of_blur[i])), lst_of_blur[i], label='CNN with {} resblocks'.format(i+1))
#     plt.legend
...
#     plt.xlabel("epochs")
#     plt.ylabel("loss of validation")
#     plt.title("super: loss for resblocks num")
#     for i in range(len(lst_of_super)):
#         plt.plot(range(len(lst_of_super[i])), lst_of_super[i], label='CNN with {} resblocks'.format(i+1))
#     plt.legend
...
# Answer1 = "I wiil anser it in external note" #@param {type:"string"}
...
# Answer2 = "I wiil anser it in external note" #@param {type:"string"}
...
# Answer3 = "I will anser it in external note" #@param {type:"string"}
